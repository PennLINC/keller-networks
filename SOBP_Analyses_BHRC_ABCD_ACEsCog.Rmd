...
# Calculate Thompson factor scores, remove outliers, calculate total ACEs
```{r}
...
# https://www.sciencedirect.com/science/article/pii/S2352289518300821
...
```{r}
summary(lmer(thompson_PC1~interview_age+sex+(1|rel_family_id)+totalACEs+totalACEs*sex,data=abcd.data))
summary(lmer(thompson_PC2~interview_age+sex+(1|rel_family_id)+totalACEs+totalACEs*sex,data=abcd.data))
summary(lmer(thompson_PC3~interview_age+sex+(1|rel_family_id)+totalACEs+totalACEs*sex,data=abcd.data))

abcd.data$ACEs.binary <- factor(abcd.data$totalACEs>0,labels = c("More Threat","Less Threat"))
abcd.data$ACEs.binary.numeric <- as.numeric(abcd.data$ACEs.binary)

summary(lmer(ACEs.binary.numeric~interview_age+sex+(1|rel_family_id)+thompson_PC1+thompson_PC2+thompson_PC3,data=abcd.data))

summary(lmer(acceptance~interview_age+sex+(1|rel_family_id)+thompson_PC1+thompson_PC2+thompson_PC3,data=abcd.data))
summary(lmer(monitoring~interview_age+sex+(1|rel_family_id)+thompson_PC1+thompson_PC2+thompson_PC3,data=abcd.data))


summary(lmer(thompson_PC1~interview_age+sex+(1|rel_family_id)+ACEs.binary.numeric+acceptance+monitoring,data=abcd.data))

summary(lmer(thompson_PC2~interview_age+sex+(1|rel_family_id)+ACEs.binary.numeric+acceptance+monitoring,data=abcd.data))

summary(lmer(thompson_PC3~interview_age+sex+(1|rel_family_id)+ACEs.binary.numeric+acceptance+monitoring,data=abcd.data))


ggplot(data=abcd.data,aes(totalACEs,thompson_PC1))+geom_point(size=1,colour=abcd.data$interview_age)+geom_smooth(method=lm)+ facet_wrap(~ factor(sex,labels=c("Male","Female")))
```
