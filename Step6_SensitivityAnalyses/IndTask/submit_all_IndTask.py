...
tmpdir = '/cbica/projects/abcdfnets/scripts/keller_ridge/pennlinc-regression/example_keller/tempfiles_IndTask' #os.environ['TMPDIR']
networks = np.array(['all']) 
network = 'all'

GB = '200G'    

for cog_score in ['PicVocab','Flanker','Pattern','Reading','Picture','List','CardSort','LMT','RAVLT']:
        os.system('qsub -l h_vmem={0},s_vmem={0} -N {1} -R y -V -j y -b y -o ~/sge/ -e ~/sge/ python {2}/keller_proc_predict_IndTask.py {2} {3} {4} {5}'.format(GB,cog_score,homedir,tmpdir,network,cog_score))
