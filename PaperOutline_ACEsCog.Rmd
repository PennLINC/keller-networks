...
# Verify that correlation is low: it's 0.222
cor.test(all.data.complete$depriv,all.data.complete$emo_neglect)
...
# Test specificity
summary(lmer(EF~age+gender+(1|state)+(1|schoolID)+emo_neglect+depriv,data=all.data.complete.ageres)) # both sig
summary(lmer(WM~age+gender+(1|state)+(1|schoolID)+emo_neglect+depriv,data=all.data.complete.ageres)) # both sig
summary(lmer(TP~age+gender+(1|state)+(1|schoolID)+emo_neglect+depriv,data=all.data.complete.ageres)) # both sig 
summary(lmer(IC~age+gender+(1|state)+(1|schoolID)+emo_neglect+depriv,data=all.data.complete.ageres)) # just depriv 
...
abcd.data<-merge(abcd.data.almost,neglect.data[,c("subjectkey", setdiff(colnames(neglect.data),colnames(abcd.data.almost)))],by="subjectkey")
...
```{r}
summary(lmer(thompson_PC1~interview_age+sex+(1|rel_family_id)+acceptance+monitoring,data=abcd.data))
summary(lmer(thompson_PC2~interview_age+sex+(1|rel_family_id)+acceptance+monitoring,data=abcd.data))
summary(lmer(thompson_PC3~interview_age+sex+(1|rel_family_id)+acceptance+monitoring,data=abcd.data))
